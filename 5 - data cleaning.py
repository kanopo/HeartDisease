...
data = data.dropna(axis=1, thresh=min_count_cols)        # droppo colonne che hanno meno valori non nulli rispetto alla tresh

min_count_raws = int(((100 - perc_drop_na)/100)*data.shape[1]+1)
data = data.dropna(axis=0, thresh=min_count_raws)        # droppo righe che hanno meno valori non nulli rispetto alla tresh
